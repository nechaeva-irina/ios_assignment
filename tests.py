...
def test_ios_switches(init_driver, json_switches):
    testdata_model = json_switches
    init_driver.find_element(By.ID, 'Switches').click()
    init_driver.find_element(AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSwitch[2]').set_value(testdata_model.switches_value)
    tinted = init_driver.find_element(AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSwitch[2]')
    tinted_value = tinted.get_attribute("value")
    init_driver.back()
    assert tinted_value == "0"
...
#     for i in range(0, testdata_model.steppers_value):
#         el.click()
#     init_driver.back()
